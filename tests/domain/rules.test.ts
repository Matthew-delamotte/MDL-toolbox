...
import { teamVoiceIssues } from "../../src/lib/providers/outreach";
...
describe("offer generation validation and factual safeguards", () => {
  ...
  it("uses conditional French outreach and a stop footer", async () => { const ai = new DevelopmentAIService(); const offer = await ai.generateOffer(context, templates); const draft = await ai.draftOutreach(context, offer, { senderName: "Matthew de Lamotte", companyName: "MDL Advisory", signature: "Matthew de Lamotte\nMDL Advisory" }); expect(draft.language).toBe("fr"); expect(draft.body).toContain("?"); expect(draft.body).not.toMatch(/\bvous (perdez|passez) du temps\b/); expect(teamVoiceIssues(draft.body)).toEqual([]); expect(draft.body).toContain("ne vous recontacterai plus"); expect(draft.body.split(/\s+/).length).toBeLessThanOrEqual(125); });
});
