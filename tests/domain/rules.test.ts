...
describe("offer generation validation and factual safeguards", () => {
  ...
  it("uses conditional French outreach and a stop footer", async () => { const ai = new DevelopmentAIService(); const offer = await ai.generateOffer(context, templates); const draft = await ai.draftOutreach(context, offer, { senderName: "Matthew", companyName: "MDL Advisory", signature: "Matthew\nMDL Advisory" }); expect(draft.language).toBe("fr"); expect(draft.body).toMatch(/\bSi \S/); expect(draft.body).not.toMatch(/\bvous (avez|utilisez|perdez)\b/); expect(draft.body).toContain("ne vous recontacterai plus"); expect(draft.body.split(/\s+/).length).toBeLessThanOrEqual(125); });
});
